...
const PYUSD_SEPOLIA_ADDRESS = "0xCaC524BcA292aaade2DF8A05cC58F0a65B1B3bB9" as const
const WPYUSD_AMOY_ADDRESS = "0xDCD5c55a144E325274508eC3bEf0d8e29E2F1cfE" as const // wPYUSD on Amoy
...
//   setShowTopupFlow,
  ...
  const isCorrectChain = isSepoliaChain || isAmoyChain
  ...
  // Fetch wPYUSD balance on Polygon Amoy
  const { 
    data: wpyusdBalance, 
  ...
  // Mint function (only available on Sepolia for testing)
  ...
  }, [isSuccess, refetchPyusdBalance, refetchWpyusdBalance])
  ...
  return (
    <Card className="border-gray-800/50 bg-gradient-to-br from-gray-900/40 to-black/40 backdrop-blur-sm hover:border-gray-700/50 transition-all duration-300">
      <CardHeader className="pb-4">
        <CardTitle className="flex items-center justify-between">
          ...
          <Button
            ...
            disabled={isPyusdLoading || isWpyusdLoading}
            className="h-8 w-8 p-0 hover:bg-gray-800/50"
          >
            <RefreshCw className={`w-4 h-4 text-gray-400 ${(isPyusdLoading || isWpyusdLoading) ? 'animate-spin' : ''}`} />
          </Button>
        </CardTitle>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="p-4 rounded-xl bg-gradient-to-r from-purple-900/20 to-purple-800/20 border border-purple-500/20">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-medium text-gray-300">wPYUSD</span>
            </div>
        ...
        {isConnected && (
          <div className="pt-2">
            {!isSepoliaChain && !isAmoyChain ? (
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                <p className="text-sm text-amber-400 flex items-center">
                  <Zap className="w-4 h-4 mr-2" />
                  Switch to Sepolia or Amoy to interact with tokens
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="p-3 bg-gray-800/30 rounded-lg border border-gray-700/50">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-blue-400 font-medium">Ready</span>
                  </div>
                </div>
              </div>
            )}
